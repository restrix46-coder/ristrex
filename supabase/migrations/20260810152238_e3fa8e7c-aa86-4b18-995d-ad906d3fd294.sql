CREATE TABLE public.message_sync_events (
...
GRANT SELECT, INSERT ON public.message_sync_events TO authenticated;
GRANT ALL ON public.message_sync_events TO service_role;
ALTER TABLE public.message_sync_events ENABLE ROW LEVEL SECURITY;
CREATE POLICY "message_sync_events_own_read" ON public.message_sync_events FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "message_sync_events_own_insert" ON public.message_sync_events FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE INDEX message_sync_events_project_created_idx ON public.message_sync_events(project_id, created_at DESC);

DELETE FROM public.messages older USING public.messages newer
WHERE older.project_id = newer.project_id
  AND NULLIF(older.parts->>'id', '') IS NOT NULL
  AND older.parts->>'id' = newer.parts->>'id'
...
CREATE UNIQUE INDEX IF NOT EXISTS messages_project_position_unique ON public.messages(project_id, position);
...
BEGIN
  ...
  FOR item, idx IN SELECT value,(ordinality-1)::integer FROM jsonb_array_elements(p_messages) WITH ORDINALITY LOOP
    ...
    VALUES(p_project_id,p_user_id,COALESCE(item->>'role','assistant'),item,idx)
...
BEGIN
  PERFORM pg_advisory_xact_lock(hashtextextended(p_project_id::text,0));
  IF NOT EXISTS(SELECT 1 FROM public.projects WHERE id=p_project_id AND user_id=p_user_id) THEN RAISE EXCEPTION 'project_not_found_or_forbidden'; END IF;
  IF message_id IS NOT NULL AND EXISTS(SELECT 1 FROM public.messages WHERE project_id=p_project_id AND parts->>'id'=message_id) THEN RETURN jsonb_build_object('ok',true,'skipped',true,'reason','message_already_exists'); END IF;
...
END; $$;
GRANT EXECUTE ON FUNCTION public.save_conversation_atomic(uuid,uuid,jsonb) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.append_message_atomic(uuid,uuid,jsonb) TO authenticated, service_role;